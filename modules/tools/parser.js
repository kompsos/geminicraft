...
const language = require("minecraft-data")(config.version).language;
...
function parseJson(json, parent) {
  ...
  if (json.translate) {
    // I checked with the native minecraft code. This is how Minecraft does the matching and group indexing. -hhhzzzsss
    if (language[json.translate]) {
      const _with = json.with || [];
      let i = 0;
      raw += language[json.translate].replace(
        /%(?:(\\d+)\\$)?(s|%)/g,
        (g0, g1) => {
          if (g0 === "%%") {
            return "%";
